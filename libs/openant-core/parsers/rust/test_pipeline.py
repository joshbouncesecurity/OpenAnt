#!/usr/bin/env python3
"""
Rust Parser Pipeline

Tests the Rust parser pipeline components:
1. RepositoryScanner - Enumerates .rs files
...
class RustPipelineTest:
    ...
    def run_parser_pipeline(self) -> bool:
        """Run the full Rust parser pipeline (scan, extract, call graph, generate)."""
        ...
        print("=" * 60)
        print("STAGE: rust_parser_pipeline")
        print("=" * 60)
        print()
        ...
        try:
            # Stage 1: Scan
            print("  [1/4] Scanning repository for Rust files...")
            ...
            # Save call graph for reachability filter
            ...
            self.results['stages']['rust_parser'] = result
            return True
        ...
        try:
            ...
            for func_id, func_data in functions.items():
                normalized_functions[func_id] = {
                    ...
                    'isExported': func_data.get('isExported', func_data.get('is_public', True)),
                    'isPublic': func_data.get('isPublic', func_data.get('is_public', False)),
                    'isAsync': func_data.get('isAsync', func_data.get('is_async', False)),
                }
    ...
    def run_context_enhancer(self) -> bool:
        ...
        print("=" * 60)
        print(f"STAGE: context_enhancer (Rust, {mode} mode)")
        print("=" * 60)
        print()
...
def main():
    parser = argparse.ArgumentParser(
        description='Run the Rust parser pipeline on a repository',
...
"""
    ...
    parser.add_argument(
        'repo_path',
        help='Path to the Rust repository to analyze'
    )
    parser.add_argument(
        '--output', '-o',
