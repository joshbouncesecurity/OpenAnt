#!/usr/bin/env python3
"""
Repository Scanner for Rust Codebases

Enumerates ALL Rust source files in a repository for complete coverage.
This is Phase 1 of the Rust parser - file discovery.
...
Output (JSON):
    {
        ...
        "files": [
            { "path": "relative/path/to/file.rs", "size": 1234 }
        ],
        "statistics": {
            "total_files": 150,
...
class RepositoryScanner:
    """
    Scan a repository for all Rust source files.

    This is Stage 1 of the Rust parser pipeline. It walks the directory tree,
    identifies Rust source files, and collects metadata about each file.

    Key features:
    - Excludes common non-source directories (target, .cargo, .git, etc.)
    - Optionally skips test files (tests/, *_test.rs, #[test] modules)
    ...
    Attributes:
        ...
        source_extensions: Set of file extensions to include (default: {'.rs'})
    ...
    def __init__(self, repo_path: str, options: Optional[Dict] = None):
        ...
        # Default exclude patterns for Rust projects
        self.exclude_patterns: Set[str] = set(options.get('exclude_patterns', [
            '.git',
            'target',           # Rust build output
            '.cargo',           # Cargo cache
            'vendor',           # Vendored dependencies
            'node_modules',     # If mixed with JS (e.g., wasm projects)
            '.cache',
            'doc',
            'docs',
            'examples',         # Usually not production code
            'benches',          # Benchmarks
        ]))
        ...
        self.test_patterns = {
            ...
            '_test.rs',         # Test file suffix
            'test_',            # Test file prefix (less common in Rust)
            '/tests.rs',        # Module test file
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Scan a Rust repository for source files',
...
Examples:
  ...
  python repository_scanner.py /path/to/repo --skip-tests
    ...
    if args.exclude:
        ...
        default_excludes = [
            '.git', 'target', '.cargo', 'vendor', 'node_modules',
            '.cache', 'doc', 'docs', 'examples', 'benches',
        ]
