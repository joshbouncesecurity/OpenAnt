#!/usr/bin/env python3
"""
Unit Generator for Rust Codebases
...
This is Phase 4 of the Rust parser - dataset generation.
...
# File boundary marker for enhanced code (Rust uses // comments)
FILE_BOUNDARY = '\n\n// ========== File Boundary ==========\n\n'
...
class UnitGenerator:
    ...
    This is Stage 4 (final stage) of the Rust parser pipeline.
    """
    ...
    def __init__(self, call_graph_data: Dict, options: Optional[Dict] = None):
        ...
        self.classes = call_graph_data.get('classes', {})  # impl blocks
    ...
    def create_unit(self, func_id: str, func_data: Dict) -> Dict:
        ...
        class_name = func_data.get('class_name')  # impl block name in Rust
        ...
        unit = {
            ...
            'metadata': {
                'is_public': func_data.get('is_public', False),
    ...
    def generate_analyzer_output(self) -> Dict:
        ...
        for func_id, func_data in self.functions.items():
            functions[func_id] = {
                ...
                'isPublic': func_data.get('is_public', False),
                'isAsync': func_data.get('is_async', False),
                'isExported': func_data.get('is_public', False),  # Rust: public = exported
                'moduleName': func_data.get('module_name'),
                'parameters': func_data.get('parameters', []),
                'className': func_data.get('class_name'),  # impl block name
            }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Generate analysis units from Rust call graph data',
