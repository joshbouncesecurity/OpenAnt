...
class _FakeClient:
    ...
    def analyze_sync(self, prompt, system=None, max_tokens=None, model=None):
    ...
    def get_last_call(self):
        return {"input_tokens": 10, "output_tokens": 50, "cost_usd": 0.0}


@pytest.fixture
def patched_anthropic(monkeypatch):
    monkeypatch.setenv("ANTHROPIC_API_KEY", "sk-test-key")
    monkeypatch.setattr(generator, "AnthropicClient", _FakeClient)
...
def test_generate_disclosure_prompt_has_no_source_code(patched_anthropic, pipeline_output):
    ...
    prompt = _FakeClient.last_prompt
